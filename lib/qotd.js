...
class Qotd {
  constructor(options = {}) {
    ...
    // display Title
    process.stdout.write(chalk.underline.green.bold('\n\nQuote of the day!\n\n'));
    ...
    if (this.options.offline) {
      // remove spinner for offline quote
      this.spinner = null;
      this.offlineQuote();
      return;
    }
  ...
  initializeSpinner() {
    ...
    this.instanceCheck('initializeSpinner');
    ...
    this.spinner.text = 'Checking for internet connectivity...';
    this.spinner.start();
  }

  async checkOnline() {
    // check if constructed
    this.instanceCheck('checkOnline');
  ...
  onlineFailSafe() {
    // check if constructed
    this.instanceCheck('onlineFailSafe');
  ...
  fetchQuote() {
    ...
    this.spinner.text = 'Fetching Quote...';
  ...
  selectQuote() {
    // check if constructed
    this.instanceCheck('selectQuote');
  ...
  outputQuote(quoteObj) {
    // check if constructed
    this.instanceCheck('outputQuote');
    ...
    if (colors.enabled) {
      ...
      const quoteOuput = colors
        .quote
        .split('.')
        .reduce((a, b) => a[b], chalk);

      const authorOuput = colors
        .author
        .split('.')
        .reduce((a, b) => a[b], chalk);
  ...
  outputCount() {
    // check if constructed
    this.instanceCheck('outputCount');
    ...
    const length       = quotesJson.quotes.length;
  ...
  offlineQuote() {
    // check if constructed
    this.instanceCheck('offlineQuote');
    ...
    if (this.spinner) {
      ...
      spinner.color = 'red';
      spinner.text = 'Selecting from Existing DataStore';
    }
  ...
  quotesLengthComparator() {
    // check if constructed
    this.instanceCheck('quotesLengthComparator');
  ...
  prepOutputJson() {
    // check if constructed
    this.instanceCheck('prepOutputJson');
    ...
    const output = Object.assign({}, quotesJson);
  ...
  writeUpdate() {
    // check if constructed
    this.instanceCheck('writeUpdate');
  ...
  instanceCheck(methodName = 'instanceCheck') {
    if (typeof this.constructed === 'undefined') {
      // static method calls attemtped
      throw new ReferenceError(`Must create an instance before calling: ${methodName}.`);
    }
  }
}
